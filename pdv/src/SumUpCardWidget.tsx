...
export function SumUpCardWidget({
    ...
    void loadSumUpScript()
      .then(() => {
        ...
        if (window.SumUpCard?.mount) {
          ...
          throw new Error('Widget SumUp indisponivel');
        }
        setReady(true);
      })
      .catch((err) => {
  ...
  if (error) {
    return <p className="pdv-pix-error">{error}</p>;
  }

  return (
    <div>
      {!ready && <p className="pdv-pix-hint">Carregando widget SumUp...</p>}
      <div id={containerId} className="pdv-sumup-host" />
    </div>
  );
}
